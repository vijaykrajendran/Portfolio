...
const ROADMAP = [
  ...
  {
    week: 2,
    title: 'Context, Control & Orchestration',
    days: 'Days 8–14',
    steps: [
      {
        id: 5,
...
export default function AIDevOpsTracker() {
  ...
  const progCount = allSteps.filter(
    s => progress[s.id] === 'in_progress',
  ).length;
  const leftCount = allSteps.length - doneCount - progCount;
  const pct = Math.round((doneCount / allSteps.length) * 100);
  const blockBar = Array.from(
    { length: 10 },
    (_, i) => i < Math.round(pct / 10),
  );
  ...
  return (
    <>
      ...
      <main className='main-tracker'>
        {ROADMAP.map(week => {
          const wpDone = week.steps.filter(
            s => progress[s.id] === 'done',
          ).length;
          ...
          return (
            <section key={week.week} className='week'>
              <div className='week-header'>
                ...
                <span className='week-pct' style={{ color: wpColor }}>
                  {wpPct}%
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        onKeyDown={e =>
                          e.key === 'Enter' && toggleStep(step.id)
                        }
                      >
      ...
      <footer className='site-footer'>
        ...
        <div>
          <a
            href='https://www.youtube.com/watch?v=Yb-asGyMM8A'
            target='_blank'
